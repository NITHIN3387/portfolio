...
const Navbar = () => {
  ...
  return (
    <div className="flex justify-between items-stretch w-[calc(100%-5px)] backdrop-blur-xl p-5 top-0">
      ...
      <Image src={require('../assets/images/menu.png')} alt="menu" width={45} className="sm:hidden inline" onClick={() => setToggle(true)}/>
      ...
      <div className={(toggle ? "grid nav-open" : "hidden nav-close") + ' grid-cols-[2fr_12fr] h-[100lvh] w-full absolute left-0 top-0'}>
        <div className="bg-black/50 flex justify-center" onClick={() => setToggle(false)}>
          <span style={{zoom: 3}}></span>
        </div>
        <ul className=" sm:hidden gap-10 pt-10 font-extrabold flex flex-col items-center bg-black">
          {
              navlist.map((list) => (
                  <Link key={list.title} href={list.link} className="flex justify-center items-center text-[1.25em]" onClick={() => setToggle(false)}>{list.title}</Link>
              ))
          }
        </ul>
      </div>
